...
# 14 Sep 2020 Few tweaks from experience of using an old version while on holiday with zombie. This version
#             may still be useful for zombie with it's 'limited' processing power. 
#             NB. pre-existing files are not deleted
...
# perl it2gmp.pl -p "N:\Music\playlists\music.xml" -m "k:\Music"
# perl it2gmp.pl -m "V:\mediaplayer\Music" -s 10GB
...
my @plItems = $pl->items();  # "$pl->items()" seems to be an array...
my $plcount = @plItems;
my $trackcnt = 0;
my $bytecount = 0;
my $trackidx = 0;
...
while($trackcnt < $tracktotal)
{
   ...
   elsif(($maxbytes>0) && ($bytecount >= $maxbytes))
   {
   	$log->debug("Max. bytecount exceeded: %d\n", $bytecount);
   	last;
   }
   if($rndtrk > 0)
   {
      $track = $plItems[int rand@plItems]; # this bit of Perl magic should randomly select an item from the list
   }
   else
   {
      $track = $plItems[$trackidx]; 
   }
   $trackidx++;
   ...
   # More reliable to sanitizze after getFSname
   ...
   $log->debug("Track %s|%s|%s Location: %s\n", $track->artist(), $albumFS, $track->name(), $srcpathFS);

   # Destination path name is artist\album-trackfilename 
   ...
   $destpathFS =~ s/\.[^\.]*$/.mp3/g; 
   $log->debug("MP Album dir: %s Dest Filepath: %s\n", $mpalbumdirFS, $destpathFS);
   ...
   eval
   {
      ...
      unless(-f $destpathFS)
      {
         $log->info("Copying %s - %s - %s to %s\n", $track->artist(), $track->album(), $track->name(),  $destpathFS);
         ...
         if($srcpathFS =~ m/.*\.m4a$/i)
      	{
      	   ...
      	   # ffmpeg -i <infile>.m4a -c:v copy -map 0 -metadata:s:v title="cover" -metadata:s:v comment="Cover (Front)"  -id3v2_version 3 -write_id3v1 1 -codec:a libmp3lame -q:a 0 <outfile>.mp3
      	   # Multiple copies of the script can be started to speed conversion. This can result
      	   # in ffmpeg being called twice for the same file which causes one to hang waiting for
      	   # user input. So ffmpeg;
      	   #    must not prompt for user input if a file already exists (-nostdin)
      	   #    must not overwrite an existing file (-n)
      	   $ffmpegcmd = sprintf("\"%s\" -hide_banner -nostdin -n -nostats -loglevel error -i \"%s\" -c:v copy -map 0 -metadata:s:v title=\"cover\" -metadata:s:v comment=\"Cover (Front)\" -id3v2_version 3 -write_id3v1 1 -codec:a libmp3lame -q:a 0 \"%s\"", $FFMPEG, $srcpathFS, $destpathFS);
      	   $log->debug("Conversion command: %s\n", $ffmpegcmd);
      	   system($ffmpegcmd);
      	   $trackcnt++;
